// Import and require Inquirer, Express, MySQL2, Console Table, and 
const inquirer = require("inquirer");
const express = require('express');
const mysql = require('mysql2');
const cTable = require("console.table");

// Environment port or Local port 3001
...
function updateEmployeeRole() {
  ...
  .then(function(answer) {
    db.query('UPDATE employee SET role_id=? WHERE ?=employee.id',[answer.updateEmployeeRole, answer.employeeId],function(err, res) {
      if (err) throw err;
      console.table(res)
    })
  })
  .then(function (answer) {
    if (answer.updateEmployeeRole === 2 || 4 || 6 || 8) {
      db.query('UPDATE employee SET manager_id =? WHERE ?=employee.id',[answer.updateEmployeeRole -1, answer.employeeId],function(err, res) {
        if (err) throw err;
        console.table(res);
        firstPrompt();    
      }
    )}else if (answer.updateEmployeeRole === 1 || 3 || 5 || 7) {
      db.query('UPDATE employee SET manager_id = NULL WHERE ?=employee.id',[answer.employeeId],function(err, res) {
        if (err) throw err;
        console.table(res);
        firstPrompt();
      })
